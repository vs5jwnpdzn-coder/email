...
<head>
  ...
  <style>
    * { box-sizing: border-box; font-family: Arial, sans-serif; }
    ...
    button { width:100%; padding:10px; margin-top:15px; border:none; cursor:pointer; color:#fff; font-size:15px; border-radius:5px; background:#2196F3; }
...
<body>
  <div class="container">
    ...
    <form action="register.php" method="POST" onsubmit="return checkPasswords()">
      ...
      <input type="password" id="pw2" name="confirm_password" placeholder="Passwort wiederholen" required>
      <button type="submit">Registrieren</button>
    </form>

    <div class="link">
      Schon einen Account? <a href="index.php">Jetzt anmelden</a>
    </div>
  </div>
  ...
  <script>
    function checkPasswords() {
      ...
      if (pw !== pw2) { alert("Passwörter stimmen nicht überein!"); return false; }
      return true;
    }
    ...
    if (error === "1") { box.textContent = "Benutzername ist bereits vergeben."; box.style.display = "block"; }
    else if (error === "2") { box.textContent = "Bitte alle Felder ausfüllen."; box.style.display = "block"; }
    else if (error === "3") { box.textContent = "Server- oder Datenbankfehler. Bitte später erneut versuchen."; box.style.display = "block"; }
    else if (error === "4") { box.textContent = "Passwörter stimmen nicht überein."; box.style.display = "block"; }
