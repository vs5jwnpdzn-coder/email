  ...
  try {
    ...
    if (req.method === "DELETE" && action === "clear-emails") {
      ...
      try {
        ...
        for (const item of raw || []) {
          const parsed = parseEmailItem(item);
          if (parsed) await kv.del(usedKey(username, parsed.email));
        }
      } catch {}
    ...
    if (req.method === "PATCH" && action === "email-used") {
      ...
      const key = usedKey(username, email);

      if (used) await kv.set(key, 1);
    ...
    if (req.method === "PATCH" && action === "user") {
      ...
      try {
        for (const item of rawEmails || []) {
          const parsed = parseEmailItem(item);
          if (!parsed) continue;
          const oldK = usedKey(oldUsername, parsed.email);
          const newK = usedKey(newUsername, parsed.email);
          const used = await kv.get(oldK).catch(() => null);
          if (used) await kv.set(newK, 1);
          await kv.del(oldK);
        }
      } catch {}
